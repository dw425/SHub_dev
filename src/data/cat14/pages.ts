...
const pages: PageData[] = [
  {
    ...
    algorithms: {
      ...
      cards: [
        { icon: '📌', title: 'Gateway Architecture', subtitle: '', description: 'How API gateways fit into your system design', tags: [] },
        { icon: '📌', title: 'Gateway Patterns', subtitle: '', description: 'Different deployment models for different needs', tags: [] },
        { icon: '📌', title: 'Authentication & Authorization', subtitle: '', description: 'Securing access at the gateway level', tags: [] },
        { icon: '📌', title: 'Traffic Management', subtitle: '', description: 'Rate limiting, throttling, and quota enforcement', tags: [] },
        { icon: '📌', title: 'Load Balancing', subtitle: '', description: 'Distributing traffic across service instances', tags: [] },
        { icon: '📌', title: 'Observability & Monitoring', subtitle: '', description: 'Visibility into every request through your gateway', tags: [] },
        { icon: '📌', title: 'Gateway Tools Comparison', subtitle: '', description: 'Popular API gateway solutions', tags: [] },
        { icon: '📌', title: 'Configuration Example', subtitle: '', description: 'Kong Gateway declarative configuration', tags: [] },
      ],
    },
    tools: {
    ...
    bestPractices: {
      ...
      doItems: [
        'Keep Gateway Stateless — Store session data in external stores (Redis, Memcached). This enables horizontal scaling and zero-downtime deployments.',
        'Implement Circuit Breakers — Prevent cascade failures by detecting unhealthy services and failing fast. Allow recovery time before retrying.',
        'Use Declarative Configuration — Version control your gateway config as code. Enable GitOps workflows for audit trails and rollback capability.',
        'Add Request IDs Early — Generate unique request IDs at the gateway and propagate through all services for end-to-end tracing.',
        'Cache Aggressively — Cache responses at the gateway for read-heavy APIs. Reduces backend load and improves latency significantly.',
        'Monitor Gateway Health — Track latency percentiles, error rates, and throughput. Set up alerts for degradation before it impacts users.',
        'Plan for High Availability — Deploy multiple gateway instances across availability zones. Use health checks and automatic failover.',
        'Avoid Business Logic — Keep the gateway focused on cross-cutting concerns. Business logic belongs in services, not the gateway layer.',
      ],
      dontItems: [
      ],
    },
  ...
  },
  {
      ...
      subsections: [
        {
          ...
          paragraphs: [
            ...
            'This decoupling provides profound benefits for complex systems. Teams can develop and deploy services independently. Systems scale naturally because consumers can be added without modifying producers. Failures are isolated—if a consumer goes down, the producer continues operating, and events queue until the consumer recovers. The temporal decoupling means systems can handle traffic bursts by buffering events rather than dropping requests.',
    ...
    algorithms: {
      ...
      subtitle: 'Evaluating approaches and tools',
      cards: [
        { icon: '📌', title: 'Core Concepts', subtitle: '', description: 'Understanding event-driven architecture fundamentals', tags: [] },
        { icon: '📌', title: 'Event Patterns', subtitle: '', description: 'Architectural patterns for event-driven systems', tags: [] },
        { icon: '📌', title: 'Message Brokers', subtitle: '', description: 'Infrastructure for reliable event delivery', tags: [] },
        { icon: '📌', title: 'Delivery Guarantees', subtitle: '', description: 'Understanding message delivery semantics', tags: [] },
        { icon: '📌', title: 'Event Schema Design', subtitle: '', description: 'Structuring events for evolvability and clarity', tags: [] },
        { icon: '📌', title: 'Error Handling', subtitle: '', description: 'Managing failures in event-driven systems', tags: [] },
        { icon: '📌', title: 'Observability', subtitle: '', description: 'Monitoring and debugging event flows', tags: [] },
        { icon: '📌', title: 'Tools & Frameworks', subtitle: '', description: 'Technologies for building event-driven systems', tags: [] },
      ],
    },
    tools: {
      title: 'Tools & Frameworks',
    ...
    bestPractices: {
      ...
      doItems: [
        'Use Past Tense for Event Names — Events represent facts that have already occurred: OrderCreated, PaymentProcessed, UserRegistered—not CreateOrder or ProcessPayment.',
        'Include All Relevant Data — Events should be self-contained. Consumers shouldn\'t need to make additional API calls to get context. Include denormalized data when needed.',
        'Version Your Schemas — Include schema version in events. Use semantic versioning. Maintain backward compatibility within major versions.',
        'Add Correlation IDs — Include correlation and causation IDs to trace event chains across services. Essential for debugging distributed flows.',
        'Design for Idempotency — Consumers must handle duplicate events gracefully. Use idempotency keys and check for already-processed events before applying side effects.',
        'Events Are Immutable — Never modify published events. If you need to correct data, publish a new compensating event. This preserves audit trails and enables replay.',
        'Use Consumer Groups — Scale consumers horizontally with consumer groups. Each partition is consumed by one consumer, enabling parallel processing.',
        'Monitor Consumer Lag — Alert when consumers fall behind. Growing lag indicates capacity issues or stuck consumers that need investigation.',
        'Partition Wisely — Choose partition keys that ensure ordering where needed while distributing load evenly. Don\'t over-partition or under-partition.',
        'Keep Events Small — Large events impact throughput and storage. For big payloads, store in object storage and include references in events.',
        'Plan for Schema Evolution — Use schema registries. Add fields with defaults. Remove fields gracefully. Test compatibility before deploying changes.',
        'Test with Chaos — Simulate broker failures, network partitions, and consumer crashes. Verify your system recovers correctly and doesn\'t lose events.',
      ],
      dontItems: [
      ],
    },
  ...
  },
  {
    ...
    concepts: {
      ...
      cards: [
        ...
        },
        {
          ...
          icon: '↔️',
          title: '',
          description: 'Direct connectors between specific systems. Lower latency, simpler for 1:1 integrations. Can become complex with many connections (N² problem).',
          examples: [],
        },
        {
          className: 'concept-2',
    ...
    algorithms: {
      ...
      subtitle: 'Evaluating approaches and tools',
      cards: [
        { icon: '📌', title: 'Core Concepts', subtitle: '', description: 'Understanding data integration connectors', tags: [] },
        { icon: '📌', title: 'Connector Types', subtitle: '', description: 'Categories of integration connectors', tags: [] },
        { icon: '📌', title: 'Popular Connectors', subtitle: '', description: 'Most commonly used integration connectors', tags: [] },
        { icon: '📌', title: 'Data Flow Patterns', subtitle: '', description: 'Common connector deployment architectures', tags: [] },
        { icon: '📌', title: 'Data Transformations', subtitle: '', description: 'Processing data as it flows through connectors', tags: [] },
        { icon: '📌', title: 'Configuration Examples', subtitle: '', description: 'Setting up connectors for common scenarios', tags: [] },
        { icon: '📌', title: 'Monitoring & Error Handling', subtitle: '', description: 'Ensuring reliable data integration', tags: [] },
        { icon: '📌', title: 'Integration Platforms', subtitle: '', description: 'Tools for managing connectors at scale', tags: [] },
      ],
    },
    tools: {
    ...
    bestPractices: {
      ...
      doItems: [
        'Use Incremental Sync When Possible — Full refreshes are slow and expensive. Configure incremental sync based on timestamps or CDC to move only changed data.',
        'Handle Schema Evolution — Source schemas change. Configure connectors to handle new columns gracefully and alert on breaking changes.',
        'Secure Credentials Properly — Never hardcode credentials. Use secrets managers, rotate regularly, and apply least-privilege access.',
        'Monitor Sync Health — Set up alerts for sync failures, latency spikes, and volume anomalies. Dashboard key metrics visibly.',
        'Test in Staging First — Always test new connectors and configuration changes in a staging environment before production.',
        'Document Data Lineage — Track where data comes from and where it goes. Essential for debugging, compliance, and impact analysis.',
        'Plan for API Rate Limits — SaaS connectors hit API limits. Configure appropriate sync frequencies and handle 429 responses gracefully.',
        'Version Control Configurations — Store connector configs in Git. Enables review, rollback, and infrastructure-as-code practices.',
      ],
      dontItems: [
      ],
    },
  ...
  },
  {
      ...
      subsections: [
        {
          ...
          paragraphs: [
            ...
            'The lifecycle consists of eight phases that form two interconnected loops. The inner loop (Design → Develop → Test → Deploy) runs frequently during active development, often completing multiple cycles per sprint. The outer loop (Publish → Monitor → Version → Retire) runs less frequently but carries strategic weight—major versions, deprecation decisions, and sunset timelines happen here.',
    ...
    algorithms: {
      ...
      subtitle: 'Evaluating approaches and tools',
      cards: [
        { icon: '📌', title: 'Lifecycle Overview', subtitle: '', description: 'Understanding API lifecycle management', tags: [] },
        { icon: '📌', title: 'Lifecycle Phases', subtitle: '', description: 'Detailed breakdown of each lifecycle stage', tags: [] },
        { icon: '📌', title: 'API Versioning', subtitle: '', description: 'Strategies for evolving APIs safely', tags: [] },
        { icon: '📌', title: 'Deprecation Strategy', subtitle: '', description: 'Gracefully retiring API versions', tags: [] },
        { icon: '📌', title: 'API Governance', subtitle: '', description: 'Standards and policies for API quality', tags: [] },
        { icon: '📌', title: 'Documentation', subtitle: '', description: 'Essential documentation for API consumers', tags: [] },
        { icon: '📌', title: 'Tools & Platforms', subtitle: '', description: 'Software for API lifecycle management', tags: [] },
        { icon: '📌', title: 'Best Practices', subtitle: '', description: 'Guidelines for effective lifecycle management', tags: [] },
      ],
    },
    tools: {
      title: 'Tools & Frameworks',
    ...
    bestPractices: {
      ...
      doItems: [
        'Design First, Code Second — Write OpenAPI specs before implementation. Review designs with stakeholders. Generate code from specs, not specs from code.',
        'Never Break Backward Compatibility — Within a major version, only add—never remove or change. Reserve breaking changes for major version bumps with long deprecation windows.',
        'Version from Day One — Include version in URLs from the start, even for v1. It\'s much harder to add versioning later than to have it from the beginning.',
        'Automate Everything — Automate linting, testing, documentation generation, and deployment. Manual processes introduce errors and slow teams down.',
        'Track Usage Metrics — Know which endpoints are used, by whom, and how often. Data-driven decisions about deprecation and investment.',
        'Communicate Proactively — Announce changes early and often. Deprecation headers, email notifications, changelog updates, developer blog posts.',
        'Support Multiple Versions — Run old versions long enough for consumers to migrate. Budget for maintaining 2-3 major versions simultaneously.',
        'Treat Docs as Product — Documentation is your API\'s UX. Invest in quality, keep it updated, and measure consumer satisfaction with docs.',
      ],
      dontItems: [
      ],
    },
  ...
  },
  {
    ...
    algorithms: {
      ...
      subtitle: 'Evaluating approaches and tools',
      cards: [
        { icon: '📌', title: 'Webhook Overview', subtitle: '', description: 'Understanding event-driven notifications', tags: [] },
        { icon: '📌', title: 'Polling vs Webhooks', subtitle: '', description: 'Why webhooks win for real-time integrations', tags: [] },
        { icon: '📌', title: 'Common Event Types', subtitle: '', description: 'Events you\'ll encounter across platforms', tags: [] },
        { icon: '📌', title: 'Payload Structure', subtitle: '', description: 'Standard webhook payload anatomy', tags: [] },
        { icon: '📌', title: 'Webhook Security', subtitle: '', description: 'Protecting your endpoints from abuse', tags: [] },
        { icon: '📌', title: 'Retry Strategies', subtitle: '', description: 'Handling failed deliveries gracefully', tags: [] },
        { icon: '📌', title: 'Tools & Platforms', subtitle: '', description: 'Software for webhook management', tags: [] },
        { icon: '📌', title: 'Best Practices', subtitle: '', description: 'Guidelines for robust webhook handling', tags: [] },
      ],
    },
    tools: {
      title: 'Tools & Frameworks',
    ...
    bestPractices: {
      ...
      doItems: [
        'Verify Signatures Always — Never process a webhook without verifying its signature. This is your primary defense against forged requests and replay attacks.',
        'Return 200 Quickly — Acknowledge receipt immediately, then process asynchronously. Long-running handlers cause timeouts and unnecessary retries.',
        'Handle Idempotently — Webhooks may be delivered multiple times. Use the event ID to deduplicate—track processed IDs and skip duplicates.',
        'Use a Queue — Don\'t process webhooks synchronously in the HTTP handler. Push to a message queue (SQS, Redis, RabbitMQ) for reliable processing.',
        'Log Everything — Log the raw payload, headers, and processing results. When things go wrong, detailed logs are essential for debugging.',
        'Monitor Delivery Health — Track webhook success rates, latencies, and error types. Alert on anomalies—a spike in failures often indicates a real problem.',
        'Support Replay — Build an endpoint to re-fetch missed events using the source API. When webhooks fail, you need a recovery mechanism.',
        'Version Your Handlers — Payload schemas change over time. Version your webhook handlers and gracefully handle unknown fields or event types.',
      ],
      dontItems: [
      ],
    },
